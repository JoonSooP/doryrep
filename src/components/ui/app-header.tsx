        ...
        <nav className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-2 ml-2 pl-4 border-l dark:border-gray-600">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {user.name}
              <span className="ml-1 px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-300 text-xs">
                {user.role}
              </span>
            </span>
            <button
              onClick={toggle}
              className="text-sm text-gray-400 dark:text-gray-300 hover:text-blue-500 dark:hover:text-blue-400 px-1.5 py-0.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              title={theme === "light" ? "다크 모드로 전환" : "라이트 모드로 전환"}
            >
              {theme === "light" ? "\u263E" : "\u2600"}
            </button>
            <Link href="/settings" className="text-sm text-gray-400 dark:text-gray-300 hover:text-blue-500">
              설정
            </Link>
            <button
              onClick={handleLogout}
              className="text-sm text-gray-400 dark:text-gray-300 hover:text-red-500"
            >
              로그아웃
            </button>
          </div>
